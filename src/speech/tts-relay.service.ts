...
export class TtsRelayService implements OnModuleDestroy {
  ...
  constructor(@Inject(ConfigService) configService: ConfigService) {
    this.secretId = configService.get<string>('TENCENT_SECRET_ID') ?? '';
    this.secretKey = configService.get<string>('TENCENT_SECRET_KEY') ?? '';
    this.appId = Number(configService.get<string>('TENCENT_APP_ID') ?? 0);
    this.voiceType = Number(configService.get<string>('TENCENT_TTS_VOICE_TYPE') ?? 101001);
  }
